...
public class SceneEffectManager : MonoBehaviour, IEffectControl{
    ...
    [SerializeField] private RippleEffect1 vfxRippleEffect1;
    ...
    public void Add(SpriteRenderer renderer, int facing, Color color, float duration = 1f, bool frozenUpdate = false,
        bool useRawDeltaTime = false){
        Vector2 scale = renderer.transform.localScale;
        Add(renderer.transform.position, renderer.sprite, scale, facing, color, 2, duration, frozenUpdate,
            useRawDeltaTime);
    }
    ...
    public void Ripple(Vector3 position){
        ...
        this.vfxRippleEffect1.Emit();
    }

    public void CameraShake(Vector2 dir){
        GameManager.Instance.CameraShake(dir, 0.2f);
    }
    ...
    public void SpeedRing(Vector3 position, Vector2 dir){
        ...
        DOTween.Sequence()
            .Append(speedRing.transform.DOScale(Vector3.one * 1.2f, 1.5f).SetEase(DG.Tweening.Ease.OutCubic))
            .Join(speedRing.GetComponent<SpriteRenderer>().DOFade(0, 1.5f))
            .AppendCallback(() => { Destroy(speedRing); });
    }
}